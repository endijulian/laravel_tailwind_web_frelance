                    ...
                    <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                        ...
                        <div class="flex mt-2 flex-nowrap">
                            @forelse ($thumbnail as $item)
                                <img :class="{ 'border-4 border-serv-button': active === {{ $item->id }} }" src="{{ Storage::url($item->thumbnail) }}" alt="" @click="changeThumbnail('{{ Storage::url($item->thumbnail) }}', {{ $item->id }})" class="inline-block mr-2 rounded-lg cursor-pointer h-20 w-36 object-cover ">
                            @empty
                                ...
                                <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        @if ($service->user->detail_user->photo != null)
                                            <img src="{{ Storage::url($service->user->detail_user->photo) }}" alt="My profile" class="w-20 h-20 rounded-full ">
                                        @else
            ...
            <aside class="p-4 lg:col-span-4 md:col-span-12 md:pt-0">
                <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                    ...
                    <div class="flex items-start px-4 pt-6">
                        ...
                        @if ($service->user->detail_user->photo != null)
                            <img src="{{ Storage::url($service->user->detail_user->photo) }}" alt="My profile" class="object-cover w-16 h-16 mr-4 rounded-full">
                        @else
                    ...
                    <div class="px-4 pb-4 booking">
                        ...
                        @auth
                            <a href="{{ route('boking.landing', $service->id) }}" class="block px-12 py-4 my-2 text-lg font-semibold text-center text-white bg-serv-button rounded-xl">
                            Booking Now
                        </a>
                        @endauth
